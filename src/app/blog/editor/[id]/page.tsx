...
export default function BlogEditorPage() {
      ...
      const loadBlog = async () => {
        ...
        try {
          ...
          if (data) {
            ...
            // Parse AI-generated images from content
            if (data.content) {
              const imgRegex = /<img[^>]+class="[^"]*ai-generated-image[^"]*"[^>]*src="([^"]+)"[^>]*>/g;
              const altImgRegex = /<img[^>]+src="([^"]+)"[^>]+class="[^"]*ai-generated-image[^"]*"[^>]*>/g;
              const matches: AIImage[] = [];
              let match: RegExpExecArray | null;
              
              while ((match = imgRegex.exec(data.content)) !== null) {
                matches.push({
                  id: `existing-${matches.length}`,
                  url: match[1],
              ...
              while ((match = altImgRegex.exec(data.content)) !== null) {
                const exists = matches.some(m => m.url === match![1]);
                if (!exists) {
                  matches.push({
                    id: `existing-${matches.length}`,
                    url: match[1],
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await db.update('blogs', id, blogData);
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-6 h-12 sm:h-14 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            {blogStatus === 'published' && !isPreview && (
              <Badge className="gap-1 bg-green-100 text-green-700 text-[10px] hidden sm:flex">
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            ...
            >
              {publishing ? (
                ...
                <><Globe className="w-4 h-4 sm:mr-1.5" /><span className="hidden sm:inline">{blogStatus === 'published' ? 'Update' : 'Publish'}</span></>
              )}
            </Button>
          </div>
        </div>
